...
class DatabaseCleanerTest {
    ...
    fun cleanupOldData() = runTest {
        ...
        dbCleaner.cleanupOldData(tayyarAccountId = 1, timelineLimit = 3, notificationLimit = 3)

        // all but 3 timeline items and notifications and all references items should be gone for تيار account 1
        // items of تيار account 2 should be untouched
        expect(
    ...
    fun cleanupEverything() = runTest {
        ...
        dbCleaner.cleanupEverything(tayyarAccountId = 1)

        // everything from تيار account 1 should be gone
        // items of تيار account 2 should be untouched
        expect(
            hometimelineItems = listOf(
                2L to "2"
    ...
    private suspend fun fillDatabase() {
        ...
        db.insert(
            listOf(
                fakeHomeTimelineData(id = "2", tayyarAccountId = 2, authorServerId = "5")
            ),
            tayyarAccountId = 2
        )
    ...
    private fun expect(
        ...
        val cursor = db.query("SELECT tayyarAccountId, $idName FROM ${entity.simpleName} ORDER BY tayyarAccountId, $idName", null)
        cursor.moveToFirst()
        while (!cursor.isAfterLast) {
            val tayyarAccountId: Long = cursor.getLong(cursor.getColumnIndex("tayyarAccountId"))
            val id: String = cursor.getString(cursor.getColumnIndex(idName))
            loadedItems.add(tayyarAccountId to id)
            cursor.moveToNext()
        }
        cursor.close()
