...
import com.altayyar.app.appstore.CacheUpdater
import com.altayyar.app.appstore.EventHub
import com.altayyar.app.components.account.AccountActivity
import com.altayyar.app.components.accountlist.AccountListActivity
import com.altayyar.app.components.announcements.AnnouncementsActivity
import com.altayyar.app.components.compose.ComposeActivity
import com.altayyar.app.components.compose.ComposeActivity.Companion.canHandleMimeType
import com.altayyar.app.components.login.LoginActivity
import com.altayyar.app.components.preference.PreferencesActivity
import com.altayyar.app.components.scheduled.ScheduledStatusActivity
import com.altayyar.app.components.search.SearchActivity
import com.altayyar.app.components.systemnotifications.NotificationService
import com.altayyar.app.databinding.ActivityMainBinding
import com.altayyar.app.db.entity.AccountEntity
import com.altayyar.app.entity.Notification
import com.altayyar.app.interfaces.AccountSelectionListener
import com.altayyar.app.interfaces.ActionButtonActivity
import com.altayyar.app.interfaces.ReselectableFragment
import com.altayyar.app.pager.MainPagerAdapter
import com.altayyar.app.settings.PrefKeys
import com.altayyar.app.usecase.DeveloperToolsUseCase
import com.altayyar.app.usecase.LogoutUsecase
import com.altayyar.app.util.emojify
import com.altayyar.app.util.getParcelableExtraCompat
import com.altayyar.app.util.hide
import com.altayyar.app.util.reduceSwipeSensitivity
...
class MainActivity : BottomSheetActivity(), ActionButtonActivity, MenuProvider {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // https://github.com/altayyarapp/tayyar/issues/3251 for details.
    ...
    private fun handleIntent(intent: Intent, activeAccount: AccountEntity): Boolean {
         ...
         * - from our code as Long Intent Extra TAYYAR_ACCOUNT_ID
         * - from share shortcuts as String 'android.intent.extra.shortcut.ID'
         */
        var tayyarAccountId = intent.getLongExtra(TAYYAR_ACCOUNT_ID, -1)
        if (tayyarAccountId == -1L) {
            val accountIdString = intent.getStringExtra(ShortcutManagerCompat.EXTRA_SHORTCUT_ID)
            if (accountIdString != null) {
                tayyarAccountId = accountIdString.toLong()
            }
        }
        val accountRequested = tayyarAccountId != -1L
        if (accountRequested && tayyarAccountId != activeAccount.id) {
            changeAccount(tayyarAccountId, intent)
            return false
        }

        if (canHandleMimeType(intent.type) || intent.hasExtra(COMPOSE_OPTIONS)) {
            // Sharing to Tayyar from an external app
            if (accountRequested) {
                    ...
                    object : AccountSelectionListener {
                        override fun onAccountSelected(account: AccountEntity) {
                            ...
                            if (requestedId == activeAccount.id) {
                                ...
                                intent.putExtra(TAYYAR_ACCOUNT_ID, requestedId)
    ...
    companion object {
        ...
        private const val TAYYAR_ACCOUNT_ID = "tayyarAccountId"
         ...
         */
        @JvmStatic
        fun accountSwitchIntent(context: Context, tayyarAccountId: Long): Intent {
            return Intent(context, MainActivity::class.java).apply {
                putExtra(TAYYAR_ACCOUNT_ID, tayyarAccountId)
        ...
        fun openNotificationIntent(
            ...
            return accountSwitchIntent(context, tayyarAccountId).apply {
         ...
         * @param tayyarAccountId the id of the Tayyar account to open the screen with. Set to -1 for current account.
         ...
         */
        @JvmStatic
        fun redirectIntent(context: Context, tayyarAccountId: Long, url: String): Intent {
            return accountSwitchIntent(context, tayyarAccountId).apply {
